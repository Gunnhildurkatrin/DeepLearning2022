...
# For teachers. Change to correct path where data is stored:
...
#input should be perfectly divisible by 16
...
class UNET(nn.Module):
    ...
    def __init__(self,in_channels=3,out_channels=1,features=[64,128,256,512]):
        ...
        self.ups=nn.ModuleList() #want to be able to do model.eval and stuff like that
    ...
    def forward(self,x):
        ...
        for idx in range(0,len(self.ups),2):
            ...
            #This is only if input shape is not perfectly divisible by 16
            #if x.shape != skip_connection.shape:
              #  x=TF.resize(x,size=skip_connection.shape[2:])
            concat_skip = torch.cat((skip_connection,x),dim=1) #added along the channel dimension
...
for batch in train_loader:
    b = batch
    Blen = len(b['mask'])
    classes = np.array(torch.argmax(b['mask'],dim=1)).reshape(Blen*256*256)
...
for epoch in range(NUM_EPOCHS):
    ...
    for d in train_loader:
        ...
        # Compute loss.
        loss = loss_fn(output, targets)+dice_loss(output,targets,multiclass=True)
    ...
    if (epoch%5)==0:
        torch.save(model.state_dict(), './WeightsUNET.pt')
